...
(function () {
  ...
  const USERS = [
    { id: "cust-rahul", email: "rahul@themarkets.test", password: "demo123", name: "Rahul Sharma", role: "customer" },
    { id: "cust-priya", email: "priya@themarkets.test", password: "demo123", name: "Priya Verma", role: "customer" },
    { id: "staff-ananya", email: "staff@themarkets.test", password: "demo123", name: "Ananya Manager", role: "employee" },
    { id: "delivery-rohan", email: "delivery@themarkets.test", password: "demo123", name: "Rohan Singh", role: "delivery" },
    { id: "admin-root", email: "admin@themarkets.test", password: "demo123", name: "Root Admin", role: "admin" }
  ];

  const CART_KEY = "theMarketsCart";

  function getUser() {
    try { return JSON.parse(sessionStorage.getItem("theMarketsUser") || "null"); }
    catch (_) { return null; }
  }
  ...
  function setUser(user, remember) {
    ...
    if (remember) localStorage.setItem("theMarketsRememberedUser", JSON.stringify(safeUser));
  }
  ...
  function getStoredUser() {
    ...
    try {
      const remembered = JSON.parse(localStorage.getItem("theMarketsRememberedUser") || "null");
      if (remembered) {
  ...
  function updateUserUI() {
    ...
    document.querySelectorAll(".dashboard-topbar h1").forEach(h1 => {
      ...
      if (user && user.role === "customer" && /good (morning|afternoon|evening)/i.test(original)) {
        const prefix = original.split(",")[0];
        h1.textContent = prefix + ", " + user.name;
      }
    });

    document.querySelectorAll(".dashboard-topbar .btn--outline").forEach(el => {
  ...
  function protectPage() {
    ...
    if (!user || user.role !== required) {
      window.location.href = "../public/login.html?requiredRole=" + encodeURIComponent(required);
    }
  }
  ...
  function setupLogin() {
    ...
    const requiredRole = new URLSearchParams(window.location.search).get("requiredRole");
    const roleNotice = document.querySelector("#login-role-notice");
    if (requiredRole && roleNotice) {
      roleNotice.hidden = false;
      if ([...roleSelect.options].some(option => option.value === requiredRole)) roleSelect.value = requiredRole;
    }
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (!user) {
        if (error) {
          error.textContent = "Invalid demo credentials. Select a demo account or use the credentials shown below.";
          error.hidden = false;
        }
        return;
      }
  ...
  function setupCartPage() {
    ...
    list.querySelectorAll("[data-cart-action]").forEach(button => {
      button.addEventListener("click", function () {
        ...
        if (button.dataset.cartAction === "remove") current.splice(current.indexOf(item), 1);
        saveCart(current);
        setupCartPage();
      });
    });
